...
function UpdateButton() {
    ...
    const Version = () => {
        return (
            <input type="text" className="textBox" defaultValue={data[idx].ver}></input>
        );
    };
    const SelectUpdatetype = () => {
        return (
            <select>                
                <option value="none">{(data[idx].updatetype === 1) ? 'true': 'false' }</option>
                <option key="updatetype">{(data[idx].updatetype === 1) ? 'false' : 'true'}</option>
            </select>
        );
    };
    const Message = () => {
        return (
            <textarea className="textBox" id="msg">{data[idx].message}</textarea>
        );
    };
